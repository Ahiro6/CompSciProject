...
class Game:
    ...
    def home(self):
        ...
        #logo
        stddraw.picture(LOGO_PIC, CENTER_X, CENTER_Y + 0.25 * HEIGHT, WIDTH*0.85, HEIGHT*0.35)
    ...
    def help_screen(self):
        ...
        stddraw.text(CENTER_X, START_Y + (start_y + 0.05 * 1.25) * HEIGHT, "Close Help: [H]")
    ...
    def controls_screen(self):
        ...
        stddraw.text(CENTER_X, START_Y + (start_y + 0.05 * 1) * HEIGHT, "Close Controls: [C]")
    ...
    def main_screen(self):
        ...
        stddraw.setPenColor(stddraw.WHITE)
        stddraw.text(
            CENTER_X, START_Y + (start_y + 0.2) * HEIGHT, "Highscore: " + str(self.highscore)
        )
    ...
    def game_display(self):
        ...
        self.icon(0.65, 0.0125, f"""Health: {self.player.health}""", icon=HEART_PIC) #health bar
        self.icon(0.0125, 0.0125, f"""Ammo: {self.player.ammo}""", icon=ammo_pic) #ammo indicator

        #displays current collected powerup
        if self.player.powerup is not None:
            stddraw.picture(self.player.powerup.pic, START_X + 0.07 * WIDTH, START_Y + 0.175 * HEIGHT, self.player.size*2, self.player.size*2)
    ...
    def icon(self, start_x, start_y, text, icon=None):
        ...
        stddraw.setPenColor(stddraw.WHITE)
        stddraw.text(
            START_X + (start_x) * WIDTH + offset, START_Y + (start_y + 0.05) * HEIGHT, text
        )
    ...
    def game_over_display(self):
        ...
        stddraw.text(CENTER_X, CENTER_Y, "GAME OVER")
